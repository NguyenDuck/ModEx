...
class BehaviorManifestBuilder extends BaseManifestBuilder {
  public build(): ManifestV2 {
    const manifest = new ManifestV2(
      this.config.uuid ?? this.config.behavior?.uuid ?? this.UUIDGenerator.generateUUIDFromSeed('behavior_manifest'),
      this.version
    );
    ...
    if (
      ...
      m.dependencies.push({
        uuid: this.config.uuid ?? this.config.resources?.uuid ?? this.UUIDGenerator.generateUUIDFromSeed('resource_manifest'),
        version: this.version,
      });
    }
...
class ResourcePackManifestBuilder extends BaseManifestBuilder {
  public build(): ManifestV2 {
    const manifest = new ManifestV2(
      this.config.uuid ?? this.config.resources?.uuid ?? this.UUIDGenerator.generateUUIDFromSeed('resource_manifest'),
      this.version
    );
